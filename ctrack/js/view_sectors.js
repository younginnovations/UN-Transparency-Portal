...
view_sectors.view = function (args) {
    view_sectors.chunks.forEach(function (n) {
...
view_sectors.ajax = function (args) {
    ...
    for (var n in iati_codes.sector_names) {
        rev_sector_names[iati_codes.sector_names[n]] = n;
    }
    ...
    //            if (!iati_codes.sector_names[group]) {
    //                group = "930";
    //            } // use other if we do not know the group
    //            if (!crsg[group]) {
    //                crsg[group] = 0;
    //            }
    //            crsg[group] += crs[n];
    ...
    years.forEach(function (y) {
        ...
        var callback = function (data) {
            //console.log("fetch transactions sectors " + y);
...
//			console.log(ctrack.sectors_data);
    ...
    years.forEach(function (y) {
        var dat = {
            ...
            "limit": args.limit || -1,
            "select": "sector_group,sector_code," + ctrack.convert_str("sum_of_percent_of_budget"),
            "sector_group_not_null": 1,
            "groupby": "sector_group",
