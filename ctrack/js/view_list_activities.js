...
view_list_activities.view = function() {
  view_list_activities.chunks.forEach(function(n) {
    ctrack.chunk(n, "{spinner_in_table_row}");
  });
  ctrack.setcrumb(2);
  ctrack.change_hash();
...
view_list_activities.ajax = function(args) {
  ...
  var dat = {
    ...
    limit: args.limit || -1,
    select:
      "title,aid,funder_ref," +
      ctrack.convert_str("commitment") +
      "," +
  ...
  fetch.ajax(dat, function(data) {
    if (args.output == "count") {
      ctrack.chunk(
        args.chunk || "list_activities_count",
        data.rows[0]["count_aid"]
      );
      views.stats.calc();
    } else {
      if (args.compare) {
      ...
      for (var i = 0; i < data.rows.length; i++) {
        ...
        d.date_end = "N/A";
        if (v.day_start !== null) {
          d.date_start = fetch.get_nday(v.day_start);
        }
        if (v.day_end !== null) {
          d.date_end = fetch.get_nday(v.day_end);
        }

        d.reporting =
          iati_codes.publisher_names[v.reporting_ref] ||
          v.reporting ||
          v.reporting_ref ||
          "N/A";
        d.commitment = commafy(
          "" + Math.floor(ctrack.convert_num("commitment", v))
        );
      ...
      ctrack.chunk(
        (args.chunk || "list_activities_datas") + "_csv",
