...
//var prelink=args.link || "http://datastore.iatistandard.org/api/1/access/activity.xml?iati-identifier=";
    var prelink = args.link || "http://" + location.host + "/q.html?aid=";
...
//console.log(acts.html());
    ...
    var commafy = function (s) {
        return ("" + parseFloat(s)).replace(/(^|[^\w.])(\d{4,})/g, function ($0, $1, $2) {
...
// Adjust some tags using js
    ...
    acts.find("value").each(function (i) {
        var it = $(this);
        var c = it.attr("currency");
        if (!c) {
            c = it.parents("iati-activity").attr("default-currency");
        } // use default?
        if (c) {
            c = c.toUpperCase();
...
// change title to span_title (title tag seems to confuse browsers)
    acts.find("title").each(function (i) {
        var it = $(this);
//console.log(it.text());
    ...
    acts.find("participating-org").each(function (i) {
        var it = $(this);
        var c = it.attr("role");
        if (c) {
            c = c.toLowerCase();
    ...
    acts.find("transaction").each(function (i) {
        var it = $(this);
...
//		console.log("fixing "+tvalue.attr("value-date"))
    ...
    acts.find("budget").each(function (i) {
        var it = $(this);
    ...
    acts.find("activity-date,transaction-date,period-start,period-end").each(function (i) {
        var it = $(this);
        it.html(it.attr("iso-date"));
    });

    acts.find("result target, result actual").each(function (i) {
        var it = $(this);
        it.html(it.attr("value"));
    });


    acts.find("related-activity").each(function (i) {
        var it = $(this);
        if (it.html().length < 4) {
            it.html(it.attr("ref"));
        }
    ...
    acts.find("reporting-org").each(function (i) {
        var it = $(this);
        var t = it.attr("ref");
    ...
    acts.find("activity-status").each(function (i) {
        var it = $(this);
        var tc = it.attr("code");
    ...
    acts.find("sector").each(function (i) {
        var it = $(this);
    ...
    acts.find("transaction-type").each(function (i) {
        var it = $(this);
    ...
    acts.find("recipient-country").each(function (i) {
        var it = $(this);
...
//console.log("sorted "+sorted+" acts");
    ...
    acts.find("document-link").each(function (i) {
        var it = $(this);
        wrap_link(it, it.attr("url"), "a_" + this.tagName.toLowerCase());
    });

    acts.find("activity-website").each(function (i) {
        var it = $(this);
        wrap_link(it, it.html(), "a_" + this.tagName.toLowerCase());
    });

    acts.find("iati-identifier").each(function (i) {
        var it = $(this);
...
//	"' href='http://datastore.iatistandard.org/api/1/access/activity.xml?iati-identifier="+id+"'>xml</a>"));
            "' href='http://" + location.host + "/q.xml?aid=" + id + "' target='_blank'>xml</a>"));
        if (slug) {
            it.append($("<a class='a_slug' href='http://iatiregistry.org/dataset/" + slug + "' target='_blank'>dataset</a>"));
        }
    });

    acts.find("provider-org[provider-activity-id]").each(function (i) {
        var it = $(this);
    ...
    acts.find("receiver-org[receiver-activity-id]").each(function (i) {
        var it = $(this);
        var id = it.attr("receiver-activity-id");
        if (id) {
            wrapInner_link(it, prelink + id + postlink, "a_" + this.tagName.toLowerCase());
