...
sipac.serialize(function() {
  ...
  un_agencies_data["total_un_agencies"] = Object.keys(
    json_iati_codes.un_publisher_names
  ).length;
  un_agencies_data["un_agencies_in_iati"] = Object.keys(
    json_iati_codes.iati_un_publishers
  ).length;
  ...
  for (let y = year; y >= year - 5; y--) {
    ...
    sipac.get(
      "SELECT TOTAL(budget_value) FROM act JOIN budget USING (aid) WHERE budget =? AND ? BETWEEN cast(strftime('%Y', datetime(budget_day_start * 60 * 60 * 24, 'unixepoch')) as integer) and cast(strftime('%Y', datetime(budget_day_end * 60 * 60 * 24, 'unixepoch')) as integer)",
      ["budget", y],
      function(err, row) {
    ...
    var sql =
      "SELECT COUNT(DISTINCT aid) , TOTAL(spend) FROM act where reporting_ref NOTNULL AND ? BETWEEN cast(strftime('%Y', datetime(day_start * 60 * 60 * 24, 'unixepoch')) as integer) and cast(strftime('%Y', datetime(day_end * 60 * 60 * 24, 'unixepoch')) as integer)";
    sipac.get(sql, [y], function(err, data) {
    ...
    sql =
      "SELECT  COUNT(DISTINCT aid) AS count_aid FROM act  WHERE ? BETWEEN cast(strftime('%Y', datetime(day_start * 60 * 60 * 24, 'unixepoch')) as integer) and cast(strftime('%Y', datetime(day_end * 60 * 60 * 24, 'unixepoch')) as integer) AND day_end >= ? AND day_length IS NOT NULL";
  ...
  sipac.get(
    "SELECT TOTAL(budget_value) FROM act JOIN budget USING (aid) WHERE budget =?",
    ["budget"],
    function(err, row) {
  ...
  var sql =
    "SELECT COUNT(DISTINCT aid) , TOTAL(spend) FROM act where reporting_ref NOT NULL";
  ...
  sql =
    "SELECT  COUNT(DISTINCT aid) AS count_aid FROM act  WHERE day_start <= " +
    r +
    "  AND  ( day_end >= " +
    r +
    " OR day_end IS NULL )  AND  day_length IS NOT NULL";
  sipac.get(sql, function(err, data) {
  ...
  sipac.all(sql, function(err, data) {
    for (var code in data) {
      if (
        json_iati_codes["country"].hasOwnProperty(data[code]["country_code"])
      ) {
        arr[data[code]["country_code"]] =
          json_iati_codes["country"][data[code]["country_code"]];
      } else if (
        json_iati_codes["region"].hasOwnProperty(data[code]["country_code"])
      ) {
        arr[data[code]["country_code"]] =
          json_iati_codes["region"][data[code]["country_code"]];
      } else {
        arr["global-regional"][data[code]["country_code"]] =
  ...
  sipac.all(sql, function(err, data) {
    for (var i in data) {
      var start_year = getYearFromTimeStamp(
        parseInt(data[i]["day_start"]) * (1000 * 60 * 60 * 24)
      );
      var end_year = getYearFromTimeStamp(
        parseInt(data[i]["day_end"]) * (1000 * 60 * 60 * 24)
      );
      currentProjectTrend = getCurrentTrends(currentProjectTrend, [
        start_year,
        end_year
      ]);
    }
  ...
  sipac.all(sql, function(err, data) {
    for (var i in data) {
      if (
        ...
        sectorGroupList[data[i].sector_code] =
...
setTimeout(function generate() {
  fs.writeFileSync(
    __dirname + "/../../dstore/json/un_agencies_data.json",
    JSON.stringify(un_agencies_data, null, "\t")
  );
  ...
  chunkopts["iati_un_publishers"] = JSON.stringify(
    json_iati_codes["iati_un_publishers"]
  );
  chunkopts["country_names_json"] = JSON.stringify(
    un_agencies_data["countries"]
  );
  chunkopts["crs_countries_json"] = JSON.stringify(
    un_agencies_data["countries"]
  );
  chunkopts["total_projects"] = JSON.stringify(
    un_agencies_data["total_projects"]
  );
  chunkopts["active_projects"] = JSON.stringify(
    un_agencies_data["active_projects"]
  );
  chunkopts["total_budget"] = JSON.stringify(un_agencies_data["total_budget"]);
  chunkopts["total_expenditure"] = JSON.stringify(
    un_agencies_data["total_expenditure"]
  );
  chunkopts["total_un_agencies"] = JSON.stringify(
    un_agencies_data["total_un_agencies"]
  );
  chunkopts["un_agencies_in_iati"] = JSON.stringify(
    un_agencies_data["un_agencies_in_iati"]
  );
  ...
  var arr = arr.slice(
    arr.indexOf(currentYearStart),
    arr.indexOf(currentYearEnd)
  );
  var un_trending = {};
