...
// delete everything related to this slug
    ...
    if (acts.length == 0) // probably an org file, try and import budgets from full data
    {
        ...
        refry.tags(org, "total-budget", function (it) {
            dstore_db.refresh_budget(db, it, org, {aid: aid}, 0);
        });
        refry.tags(org, "recipient-org-budget", function (it) {
            dstore_db.refresh_budget(db, it, org, {aid: aid}, 0);
        });
        refry.tags(org, "recipient-country-budget", function (it) {
            dstore_db.refresh_budget(db, it, org, {aid: aid}, 0);
        });
...
// make really really sure old junk is deleted
        ...
        refry.tags(act, "transaction", function (it) {
            ...
            if (code == "C") {
                var usd = iati_xml.get_ex(it, "value", "USD");
                t.commitment += usd;
                var eur = iati_xml.get_ex(it, "value", "EUR");
                t.commitment_eur += eur;
                var gbp = iati_xml.get_ex(it, "value", "GBP");
                t.commitment_gbp += gbp;
                var cad = iati_xml.get_ex(it, "value", "CAD");
...
//console.log("C="+t.commitment+"\tD+E="+t.spend);
        ...
        if (!funder) {
            funder = refry.tagattr(act, {0: "participating-org", role: "funding"}, "ref");
        }
        if (funder) {
            funder = funder.trim();
        ...
        if (!funder) {
            funder = refry.tagattr(act, {0: "participating-org", role: "extending"}, "ref");
        }
        if (funder) {
            funder = funder.trim();
            if (!codes.funder_names[funder]) {
...
// fix percents to add upto 100
        ...
        var country = [];
        var percents = [];
        refry.tags(act, "recipient-country", function (it) {
            country.push((it.code || "").trim().toUpperCase());
    ...
    db.run("DELETE FROM trans WHERE trans_flags=?", 1, function (err, rows)  // remove old fake
    {
        ...
        db.all("SELECT reporting_ref , trans_code ,  COUNT(*) AS count FROM act  JOIN trans USING (aid)  GROUP BY reporting_ref , trans_code", function (err, rows) {
            for (i = 0; i < rows.length; i++) {
                ...
                else if ((v.trans_code == "D") || (v.trans_code == "E")) {
                    ids[v.reporting_ref] = (ids[v.reporting_ref] || 0) - 1;
                }
            }
            for (var n in ids) {
