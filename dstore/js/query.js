...
query.get_q = function(req) {
  ...
  var cp = function(f, unesc) {
    for (var n in f) {
      // single depth copy only
      var v = f[n];
      if (unesc) {
        v = unesc(v);
      } // use unescape function?
      if (q[n] === undefined) {
        // only set if not exists, so call cp in order of priority from high to low
        q[n] = v;
      }
    }
  };
  ...
  if (q.form == "xml" || q.form == "html") {
    // xml view so we...
    if (q.from.indexOf("jml") == -1) {
      // only add once
  ...
  if (q.select) {
    ...
    for (var i = 0; i < qq.length; i++) {
      ...
      if (calc[v]) {
        ...
        // valid member names only
        ss.push(v);
        done_list = true;
      } else {
        // try all the calc names
...
query.getsql_between = function(q, qv) {
  if (q.between) {
    ...
    //   "AND " +
    //     data[0] +
    //     " BETWEEN cast(strftime('%Y', datetime(" +
    //     data[1] +
    //     " * 60 * 60 * 24, 'unixepoch')) as integer) AND cast(strftime('%Y', datetime(" +
    //     data[2] +
    //     " * 60 * 60 * 24, 'unixepoch')) as integer)"
    // );
    return (
      "AND " +
      data[0] +
...
query.getsql_where = function(q, qv) {
      ...
      if (v !== undefined) {
        ...
        if (eq == "NOT NULL") {
          ...
          if (t == "string") {
            ...
            if (sa[0].length == 10 && sb && sb.length == 4 && ty == "int") {
              // date string, convert to number if dest is an int
              v = iati_xml.isodate_to_number(v);

              if (sa.length == 2 && /null/i.test(sa[1])) {
                // allow an explicit or |null case
                ss.push(
                  " ( " +
                    n +
                    " " +
                    eq +
                    " " +
                    dstore_db.text_plate(n + qe) +
                    " OR " +
                    n +
                    " IS NULL ) "
                );
                qv[dstore_db.text_name(n + qe)] = query.maybenumber(v, ty);
              } else {
                ss.push(
                  " " + n + " " + eq + " " + dstore_db.text_plate(n + qe) + " "
                );
                qv[dstore_db.text_name(n + qe)] = query.maybenumber(v, ty);
              }
            } else if (sa[1]) {
            ...
            qv[dstore_db.text_name(n + qe)] = v;
          }

          if (t == "object") {
            // array, string above may also have been split into array
            var so = [];
            for (var i = 0; i < v.length; i++) {
              so.push(" " + dstore_db.text_plate(n + qe + "_" + i) + " ");
              qv[dstore_db.text_name(n + qe + "_" + i)] = query.maybenumber(
                v[i],
                ty
              );
            }
            if (v.length == 2 && /null/i.test(v[1])) {
              // allow an explicit or null case for base comparisons
              ss.push(
                " (  " + n + " " + eq + so[0] + " OR " + n + " IS NULL ) "
              );
              qv[so[1].trim()] = undefined; // not going to be used
            } else if (eq == "=") {
              // make an IN statement
              ss.push(" " + n + " IN (" + so.join(",") + ") ");
            } else {
              // explode into a bunch of OR statements
              var st = [];
              st.push(" ( ");
  ...
  var v = q["text_search"];
  if (ns["title"] && ns["description"] && v) {
    // description and title and text_search available
    //console.log("text_search "+v)
    if (argv.pg) {
      // can use better pg search code
      ss.push(
        " to_tsvector('simple',title || ' ' || description) @@ plainto_tsquery('simple'," +
          dstore_db.text_plate("text_search") +
          ") "
      );
      qv[dstore_db.text_name("text_search")] = v;
    } else {
      // can only use old sqlite search code that only checks title
...
query.do_select_response = function(q, res, r) {
  ...
  if (q.form == "html") {
    ...
    var xsl =
      '<?xml-stylesheet type="text/xsl" href="/ctrack/art/activities.xsl"?>\n';
    if (q.xsl == "!") {
      xsl = "";
    } // disable pretty view
    ...
    // a jsoned csv (much smaller than json for large table data)
    if (r.rows[0]) {
      var head = [];
      var ta = [];
      for (var n in r.rows[0]) {
...
query.serv = function(req, res) {
  ...
  if (argv.instance) {
    var instance =
      (req && req.subdomains && req.subdomains[0]) || argv.instance;
