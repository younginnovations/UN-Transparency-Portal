...
// available funcs
    ...
    if (q.select) {
        ...
        for (var i = 0; i < qq.length; i++) {
            ...
            else if (ns[v]) // valid member names only
            {
                ss.push(v);
                done_list = true;
            }
                ...
                if (eq == "NOT NULL") {
                    ...
                    if (t == "string") {
                        ...
                        if (sa[0].length == 10 && sb && sb.length == 4 && ty == "int") // date string, convert to number if dest is an int
                        {
                            ...
                            {
                                ss.push(" ( " + n + " " + eq + " $" + n + qe + " OR " + n + " IS NULL ) ");
                                qv["$" + n + qe] = query.maybenumber(v, ty);
                            }
                            else {
                                ss.push(" " + n + " " + eq + " $" + n + qe + " ");
                                qv["$" + n + qe] = query.maybenumber(v, ty);
                    ...
                    if (t == "object") // array, string above may also have been split into array
                    {
                        ...
                        else if (eq == "=") // make an IN statement
                        {
...
query.getsql_group_by = function (q, qv) {
    ...
    if (q.groupby) {
        ...
        for (var i = 0; i < qq.length; i++) {
            ...
            else if ("number" == typeof n) // or number
            {
                ss.push(n + "");
            }
        }
...
query.do_select = function (q, res) {
    ...
    db.run(";", function (err, row) {
        ...
        else if (q.form == "jcsv") // a jsoned csv (much smaller than json for large table data)
        {
            if (r.rows[0]) {
                var head = [];
                var ta = [];
